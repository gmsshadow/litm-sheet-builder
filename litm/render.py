...
def render_sheet_html(character: Character, *, embed_css: bool = False) -> str:
    ...
    If `embed_css` is True the sheet CSS is inlined into a <style> block —
    useful for `file://` previews where relative paths can be fiddly.
    """
    env = _env()
    template = env.get_template("sheet.html")
    css_inline: Optional[str] = None
    if embed_css:
        css_inline = (STATIC / "css" / "sheet.css").read_text(encoding="utf-8")
